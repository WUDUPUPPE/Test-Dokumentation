...
\subsection{Einstieg in das Projekt und erste fachliche Grundlagen}
\subsubsection{19.05.26 -- Dienstag}

Nach dem Gespräch habe ich mir bereits Gedanken darüber gemacht, wie das Frontend gestaltet werden soll.

\subsubsection{20.05.26 -- Mittwoch}
...
\subsubsection{21.05.26 -- Donnerstag}
...
\subsubsection{22.05.26 -- Freitag}
...
\subsection{Umstieg auf LaTeX, Makefile und erste Bash-Skripte}
\subsubsection{26.05.26 -- Dienstag}
...
\subsubsection{27.05.26 -- Mittwoch}
...
\subsubsection{28.05.26 -- Donnerstag}
...
\subsubsection{29.05.26 -- Freitag}
...
Dann musste ich zur Firma mir einen Laptop abholen und ihn mit Linux aufsetzten und mir alles nötige beschaffen was ich brauch um vernünftig zuarbeiten

\subsection{GitHub-Workflow, Releases und Strukturierung der Repository-Logik}
\subsubsection{01.06.26 - Montag}
...
\subsubsection{02.06.26 - Dienstag}
...
\subsubsection{03.06.26 - Mittwoch}
...
\subsubsection{05.06.26 - Freitag}
...
\subsection{Frontend-Konzeptionierung für STEMgraph}
\subsubsection{08.06.26 -- Montag}
...
\subsubsection{09.06.26 -- Dienstag}
...
\subsubsection{10.06.26 -- Mittwoch}
...
\subsubsection{11.06.26 -- Donnerstag}
...
\subsubsection{12.06.26 -- Freitag}
...
\subsection{Bash-Skripte, Subgraph-Logik und Datenaufbereitung}
\subsubsection{15.06.26 -- Montag}
...
\subsubsection{16.06.26 -- Dienstag}
...
\subsubsection{17.06.26 -- Mittwoch}
...
\subsubsection{18.06.26 -- Donnerstag}
...
\subsubsection{19.06.26 -- Freitag}
...
\subsection{Testdokumentation und LaTeX-Ausarbeitung}
\subsubsection{22.06.26 -- Montag}
...
\subsubsection{23.06.26 -- Dienstag}
...
\subsubsection{24.06.26 - Mittwoch}
...
\subsubsection{25.06.26 - Donnerstag}
...
\subsubsection{26.06.26 - Freitag}
...
\subsection{Dokumentationsstruktur und Einstieg in Neo4j-Graphmodell}
\subsubsection{29.06.26 - Montag}
...
\begin{itemize}
  ...
  \item So eine deutlich klarere Grundlage geschaffen, auf der später Betreuer-Feedback und eine echte IHK-Dokumentation aufbauen können.
\end{itemize}

\subsubsection{30.06.26 - Dienstag}

War die nächste Aufgabe mir einen lokalen \texttt{Neo4j} Server per \texttt{Docker} einzurichten und mich etwas mit dem Programm vertraut zu machen

\begin{itemize}
  \item Sollte ich \texttt{Portainer} nutzen aber kam zum Entschluss das es für mich nichtz notwendig ist
  \item Hatte mich schon mal vor paar Wochen mit Youtube Videos für \texttt{Neo4j} auseinandergesetzt gehabt:
    \begin{itemize}
      \item \href{https://youtu.be/T6L9EoBy8Zk?si=zv2459hfS2osTfwN}{Neo4j in 100 Seconds}
      \item \href{https://youtu.be/REVkXVxvMQE?si=PhiNwcv7IfhSqP7F}{What is a graph database? (in 10 minutes)}
      \item \href{https://youtu.be/xKVA2gL8WHs?si=vl_GM4LRAZQXngy5}{What is Graph Database? | Neo4j Tutorial}
      \item \href{https://youtu.be/8jNPelugC2s?si=W0dSuiQ8L1hrWGmZ}{Neo4j (Graph Database) Crash Course}
      \item \href{https://youtu.be/P8VsJgbfWFw?si=x_abHF4SYK3pHdBG}{Neo4j IN 44 MINUTEN LERNEN [Full-HD] [Deutsch/German] Neo4j Crashkurs}
    \end{itemize}
\end{itemize}

Ich sollte mir die Frage stellen:

\begin{itemize}
  \item Wie muss ein json strukturiert sein, dass es als Graph erkennbar ist.
  \item Wie kannst du das in Neo4j reinladen
  \item Mache eine Entscheidungsmatrix und vergleiche die beiden Optionen:
    \begin{itemize}
      \item Crawlen, json bauen, in neo4j laden, Frontend nutzt neo4j
      \item Crawlen, json bauen, REST-API liefert direkt das json für den Graphen aus.
    \end{itemize}
\end{itemize}

\subsubsection{01.07.26 - Mittwoch}

Habe den lokalen Neo4j Server über Docker weiter eingerichtet und geprüft, ob der Container sauber läuft. 

\begin{itemize}
  \item Dafür habe ich die compose.yaml eingerichtet und mit denn typischen Docker services und denn volumes meiner wahl befüllt
  \item Im Browser die Neo4j Oberfläche geöffnet und geguckt, wie di eeinzelnen Knoten, Beziehungen und Properties dargestellt werden. 
  \item Habe erste einfache Abfragen ausprobiert, um ein Gefühl für die Oberfläche und die Query-Sprache zu bekommen.
\end{itemize}

\subsubsection{02.07.26 - Donnerstag}

Heute habe ich ein erstes Test-Graphmodell für STEMgraph in Neo4j aufgebaut. 

\begin{itemize}
  \item Drei Challenge-Knoten mit \texttt{uuid}, \texttt{teaches}, \texttt{depends\_on} und \texttt{keywords} angelegt (Linux CLI Grundlagen, Dateien und Ordner verwalten, Makefile Grundlagen). 
  \item Die Beziehungen mit \texttt{BUILDS\_ON} gesetzt, damit der Lernpfad technisch als Graph sichtbar wird.
\end{itemize}

Zusätzlich habe ich die \texttt{Neo4j} weiter ausgebaut und sie mit Daten aus der Graphdatenbank befüllt wie:

\begin{itemize}
  \item ein Datenmodell
  \item Testdaten und Beziehungen
  \item ein Graphdatenbank View
  \item ein Neo4j Test
\end{itemize}

\subsubsection{03.07.26 - Freitag}

Habe die Test-Challenges im Neo4j Browser abgefragt und mir die Abhängigkeiten im Graph angesehen. 

\begin{itemize}
  \item Gesehen, dass „Makefile Grundlagen“ auf „Dateien und Ordner verwalten“ aufbaut und diese wiederum auf „Linux CLI Grundlagen“ verweist. 
  \item So überprüft, ob die \texttt{BUILDS\_ON}-Beziehungen wie geplant funktionieren und der Lernpfad korrekt dargestellt wird.
\end{itemize}

Danach habe ich eine neue Datei \texttt{JSON Struktur Notiz} für die Skizzierung der Struktur des Graphen angelegt mit:

\begin{itemize}
  \item \texttt{nodes} und \texttt{edges}, in der jede Challenge als Node mit \texttt{id}, \texttt{teaches} und \texttt{keywords} vorkommt.
  \item Die Abhängigkeiten als \texttt{edges} über \texttt{source} und \texttt{target} abgebildet, damit später ein Graph-View im STEMgraph-Frontend möglich ist.
\end{itemize}

\subsection{Neo4j-Graphmodell, JSON-Strukturierung und Fehlerbehebung der Abhängigkeiten}
\subsubsection{06.07.26 - Montag}

hatte ich dann gestartet in der CLI das \texttt{Bash-Script} auszuführen und mir die \texttt{graph-data.json} bauen zulassen. Dabei fiel mir auf:

\begin{itemize}
  \item das in machen \texttt{edges} normale Texte war anstatt denn erwarteten id´s hatte mich gewundert
  \item hab meinen Chef gefragt ob das gewollt ist oder ob ich die weglassen soll 
\end{itemize}

Hab dann weiter gemacht mit der \texttt{JSON Notiz} und fertiggestellt um sie in die Test Doku einzufügen

\subsubsection{07.07.26 - Dienstag}

war die nächste Aufgabe die Fehlerhaften \texttt{depends\_on} in den Repo README.md zu fixen, dafür sollte ich die:

\begin{itemize}
  \item ich sollte für jedes Repo eine neues Issues auf Englisch anlegen in dem ich erkläre was denn genau in dem Repo fehlerhaft sei
  \item dann sollte ich mir die repos forken und lokal klonen um sie bearbeiten zu können
\end{itemize}

Hab dann jede README durchgesehen um die passenden  Abhängigkeit zu finden für die jeweiligen \texttt{depends\_on} in denn Repos dabei habe ich für 5 Repos passende \texttt{depend\_on} finden können und für zwei gar nicht und eins konnte ich teilweise lösen

\subsubsection{08.07.26 - Mittwoch}

da ich Dienstag die passenden Abhängigkeiten hatte hab ich mir dann jedes Repo einzeln geholt und hab:

\begin{itemize}
  \item einen eigenen Branch gesetzt \texttt{fix-depends-on}
  \item hab die passende UUID in die \texttt{depends\_on} gesetzt 
  \item commit \& push
  \item und dann denn Pull Request für die 5...
\end{itemize}

Für die README ohne Json Block habe ich einen Json-Block angelegt so weit ich alles eintragen konnte, eine README konnte ich teilweise fixen habe ich commitet \% gepushed ohne Pull-Request und die README wo ich keine Abängigkeiten finden konnte habe ich nur geklont

\subsubsection{09.07.26 - Donnerstag}

Hatte ich dann gefragt gehabt wie denn die drei am besten fixen kann , darauf war die Anweisung er muss es selbst beheben da die noch unklar sind, bis dahin war die nächste Aufgabe:

\begin{itemize}
  \item mir Tutorials zu Docker anzugucken 
  \item und mir Videos zu Angular anzugucken und mich schon mal damit vertraut zu machen  
\end{itemize}

\subsubsection{10.07.26 - Freitag}

hatte ich mich noch mehr mit Docker beschäftigt da ich einen eigenen Server habe hatte ich schon etwas Vorkenntnisse, und das hatte sich direkt angeboten mich in diesem Umfeld noch mehr damit vertraut zu machen und nachzuvollziehen was Docker ist und in welchen Szenarien man es am besten nutzt

Hatte mir heute auch ein Video angesehen wo man das benutzen von Angular selber ausprobieren kann mit viel HTML dazwischen:

\begin{verbatim}
https://youtu.be/FAtFMgodaPc?si=xk-LEOMt0-v5HXdk
\end{verbatim}
...
\subsection{Entscheidungsmatrix, Graph-Datenfix und erste Endpunkt-Planung}
\subsubsection{13.07.26 - Montag}

war die Aufgabe erstmal so mit der \texttt{graph-data.json} weiter zuarbeiten und was noch offen war zu erledigen dafür habe ich geguckt was es für relevante Kriterien in meinem Projekt gibt um sie in die Entscheidungsmatrix zupacken

\subsubsection{14.07.26 - Dienstag}

war der Plan erstmal mit der Entscheidungsmatrix zu starten da das noch eine offene Aufgabe war die zu erledigen war:

\begin{table}[h]
\centering
\begin{tabular}{|p{2cm}|p{6.5cm}|p{6.5cm}|}
\hline
\textbf{Kriterium} & \textbf{Crawlen -> Json -> Neo4j -> Frontend nutzt Neo4j direkt} & \textbf{Crawlen -> Json -> REST-API liefert JSON für den Graphen} \\
\hline
Komplexität & Hoch - Frontend muss Cypher/Neo4j kennen & Gering - Frontend spricht nur HTTP \\
\hline
Flexiblität & Hoch - Datenbankwechsel ist schwieriger & Gering - Backend austausch bar \\
\hline
Sicherheit & Datenbank ist direkt sichtbar & Datenbank ist hinter der API versteckt \\
\hline
Wartbarkeit & Weniger Schichten & Klare Trennung (Separation of Concerns) \\
\hline
\end{tabular}
\end{table}

Also meine Wahl war eigentlich die 1.Option da es einfacher für mich wäre, ohne zusätzliches Wissen umzusetzen.

Dann hatte ich das \texttt{export\_graph\_data.sh} Bash-Script so umgebaut das man jetzt auch denn Author und das Erstellunsgdatum mit exportiert und im Frontend bereitstellen kann.

\subsubsection{15.07.26 - Mittwoch}

musste ich da in einer Challenge noch ein Fehler war denn beheben in der Challenge 68ffd stand im teaches block der titel so:

\begin{itemize}
\item "Understanding the Stack in x86\_64 Assembly" mit Backslash was zu einem Fehler führte, hatte es intern erstmal nur gefixed und schreibe jetzt eine Issues und mache dann einen Pull-Request
\end{itemize}

Danch hatte ich noch mal das \texttt{export\_grah\_data.sh} Bash-Script ausgeführt um die korrigierte \texttt{graph-data.json} zu benutzen um mit APOC in Neo4j zuladen, da APOC JSON direkt parsen kann:

\begin{itemize}
\item Nodes über die id angelegt
\item und die Beziehungen über \texttt{source} und \texttt{target} verbunden
\end{itemize}

\subsubsection{16.07.26 - Donnerstag}

hab denn Betreuer von dem fehlerhaften \texttt{teaches} Eintrag informiert gehabt und danach hab ich mit denn Endpunkten gestartet und geguckt was am meisten sinn macht und was man brauchen könnte dabei kam raus:

\begin{itemize}
\item \texttt{GET /health}
\item \texttt{GET /graph}
\item \texttt{GET /challenges}
\item \texttt{GET /challenges/\{id\}}
\item \texttt{GET /subgraph?start=\{A\}\&end=\{B\}}
\item \texttt{GET /keywords}
\end{itemize}

aber dachte mir das kann ja nicht alles sein und habt etwas rechachiert was man so gänging nutzt bzw was ich in meinem fall denn alles so machen könnte

\subsubsection{17.07.26 - Freitag}

hatte ich weiter gemacht und geguckt was denn noch so gesetzt werden kann als endpoint:

\begin{itemize}
\item \texttt{GET /health}: zur Prüfung ob die FastAPI läuft und Neo4j erreichbar ist
\item \texttt{GET /graph}: Gesamter Graph
\item \texttt{GET /challenges}: Gesamter Graph
\item \texttt{GET /challenges/\{id\}}: Detaillierte Ansicht einer Challenge
\item \texttt{GET /subgraph?start=\{A\}\&end=\{B\}}: liefert nur denn relevanten Graph zwischen denn beiden Knoten
\item \texttt{GET /keywords}: zeigt alle Keywords

\item \texttt{GET /challenges/\{id\}/depends-on?format=list}: alle abhängigen Challenges als Liste
\item \texttt{GET /challenges/\{id\}/depends-on?format=tree}: alle abhängigen Challenges als Baum
\item \texttt{GET /challenges/\{id\}/neighbors}: ist für Frontend um direkt Vorgänger und Nachfolger anzuzeigen ohne denn gesamten Graph zu laden
\item \texttt{GET /keywords/\{keyword\}/challenges}: gibt mir alle Challenges mit dem gewählten Keyword
\item \texttt{POST /admin/update-challenges}: ist für denn Adminbereich um die Challenges neu zuladen bzw. um sie updaten und dann direkt als json umwandelt
\end{itemize}

\subsection{Projektantrag und Aufbau der STEMgraph-API}
\subsubsection{20.07.26 - Montag}

hatte ich einen Schultag bei dem es darum ging uns noch mal anzugucken wie wir einen Projektantrag umsetzten was wir beachten sollten und bis wann wir ihn Fertig haben sollten

\begin{itemize}
\item hab mir dafür noch mal die Unterlagen angeguckt und verinnerlicht
\item dann schon mal gestartet denn Projektantrag wieder mit LaTeX aufzusetzen
\item als nächste hatte ich die Erste Seite des Antrags geguckt wie ich sie Strukturiere
\end{itemize}

\subsubsection{21.07.26 - Dienstag}

hab ich mich ran gesetzt die API umzusetzen dafür habe ich erstmal:

\begin{itemize}
\item die Ordnerstruktur festgelegt bzw geguckt was ich brauchen könnte
\end{itemize}
...
\begin{itemize}
\item hab dann gestartet mir bestehende Lösungen anzugucken und diese zu nutzen um die API zubauen
\end{itemize}

\subsubsection{22.07.26 - Mittwoch}

war ich dann dabei die API weiter einzurichten und sinnvoll zu modellieren um sie wenn nötig zu erweitern

\begin{itemize}
\item hab dann denn Code so gut ich konnte optimiert und verkleinert
\item dann hatte ich denn Container aktiviert und getestet ob denn jeder endpunkt funktioniert was natürlich nicht sofort klappte
\end{itemize}

\subsubsection{23.07.26 - Donnerstag}

wollte ich dann endlich das die API ohne Fehler funktioniert was soweit erstml geklappt hat

\begin{itemize}
\item dann hab ich einen Termin ausgemacht gehabt um einmal alles anzuschauen
\end{itemize}

als nächstes habe ich denn Projektantrag weiter gemacht

\subsubsection{24.07.26 - Freitag}

wurde dann auch der Projektantrag weiter ausgearbeitet um eine gute Basis zu schaffen

\subsection{}
\subsubsection{27.07.26 - Montag}

erst hatte ich denn Antrag weiter ausarbeitet, hatte dann dass Gespräch zu der API dabei kam raus das

\begin{itemize}
\item der Port in der compose im neo4j noch offen ist was ja nicht sein sollte
\item das ich denn Code besser typisiere um direkt zu sehen was welcher Datentyp ist um nicht alles zu debuggen
\item und das es besser wäre mit \texttt{venv} zuarbeiten da ich dort die dependencies für jedes Projekt seperat anlegen kann und nicht im global alles installieren zumüssen
\end{itemize}

dann hab ich mich damit beschäftigt erstmal die Virtuelle Umgebung einzurichten

\begin{verbatim}
https://docs.python.org/3.11/tutorial/venv.html
\end{verbatim}

\subsubsection{28.07.26 - Dienstag}